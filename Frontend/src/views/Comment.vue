...
<template>
	<main class="container">
		...
		<div class="row mb-4 rounded">
			<p class="col-12 my-2 btn  btn-block btn-info badgeTopColor font-weight-bold" style="background-color: #138400">
		...
		<div class="row">
				...
				<div class="card mb-3">
					...
					<div class="card-body">
						...
						<div id="compteButton" class="text-center">
							<router-link v-if="isAdmin" to="/Admin"
								><button type="button" class=" btn btn-danger mx-auto rounded p-2 buttonsPanel">
									ADMIN.
					...
					<div class="justify-content-center align-self-center">
						<router-link to="/Create"
							><button type="button" class="btn btn-dark  rounded m-2 p-2 font-weight-bold ">
								CREER UNE PUBLICATION<img src="../assets/send.svg" alt="logo-envoyer" style="width: 25px" class="m-3" /></button
						></router-link>
					</div>
					<div class="justify-content-center align-self-center">
						<router-link to="/CreateComment"
							><button type="button" class="btn btn-dark  m-2 p-2 rounded font-weight-bold">
								COMMENTER<img src="../assets/write.svg" alt="write-logo" style="width: 25px" class="m-3" /></button
						></router-link>
					</div>
				</div>
			</article>
			...
			<section class="card col-12 col-md-8 bg-light mb-3">
				<div class="card-header bg-light d-flex align-items-center justify-content-between m-0 p-1">
					...
					<div class="badge bg-dark text-wrap text-white p-2" style="width: 6rem;">ref # {{ messageId }}</div>
				</div>
				<div>
					<p class="text-center">
				...
				<div class="row justify-content-between p-3">
					...
					<div v-if="isAdmin || messageUserId == currentUserId">
						<button @click="deleteMessage(messageId, messageUserId, currentUserId)" class="border-0">
		...
		<section class="row">
			<router-link to="/CreateComment"
				><p v-if="comments.length == 0" class="mt-3 btn btn-sm btn-block btn-danger font-weight-bold">
			...
			<div v-for="comment in comments" :key="comment" class="card col-12 mt-3">
				<div class="card-header ">
					<p class="btn btn-dark w-100" style="cursor:default">
				...
				<div class="card-footer row justify-content-between">
					<p class="btn btn-dark w-10 text-right mx-2 " style="cursor:default">
						ref# <span class="badge badge-light"> {{ comment.id }}</span>
					</p>
...
export default {
	...
	created: function() {
		...
		let self = this;
		axios
			.get('http://localhost:3000/api/messages/' + MessageId, { headers: { Authorization: 'Bearer ' + localStorage.getItem('token') } })
			.then(res => {
				console.log(res);
		...
		axios
			.get('http://localhost:3000/api/users/' + id, { headers: { Authorization: 'Bearer ' + localStorage.getItem('token') } })
			.then(res => {
		...
		axios
			.get('http://localhost:3000/api/comments/' + MessageId, { headers: { Authorization: 'Bearer ' + localStorage.getItem('token') } })
			.then(function(res) {
