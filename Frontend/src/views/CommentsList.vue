		...
		<div class="col-12">
			...
			<div class="col-12">
				<router-link v-if="isAdmin" to="/Admin"
					><h1 class="my-2 btn btn-block btn-danger font-weight-bold">Administration du site</h1></router-link
				>
				<Home></Home>
			</div>
					...
					<article class="card bg-light my-3 class=center-block" style="float:none;">
						...
						<div class="card-body mx-auto">
							<button class="btn btn-dark rounded" style="cursor:default">
								Pour modérer les commentaires, veuillez vous rendre sur la page des commentaires en cliquant sur le logo "oeil".
							</button>
						</div>
					</article>
				</sub>
...
export default {
	...
	created: function() {
		...
		let self = this;
		axios
			.get('http://localhost:3000/api/messages', { headers: { Authorization: 'Bearer ' + localStorage.getItem('token') } })
			.then(res => {
				console.log(res);
				if (res) {
