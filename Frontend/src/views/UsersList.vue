					...
					<div class="card bg-light my-3 class=center-block" style="float:none;">
						...
						<div class="card-body mx-auto">
							<button class="btn btn-dark rounded" style="cursor:default">
								Vous pouvez supprimer les utilisateurs que vous voulez.
							</button>
						</div>
					</div>
				</article>
				...
				<sub class="col col-md-8">
					...
					<div v-for="i in users" :key="i">
						<div class="badge btn-primary p-3 badgeList">
							...
							<span class="spanBadge">
								<button class="rounded" @click="deleteOneUser(i.id, isAdmin)">
									<img src="../assets/trash.svg" alt="logo eye" style="width:25px" />
								</button>
							</span>
						</div>
					</div>
...
export default {
	...
	created: function() {
		...
		axios
			.get('http://localhost:3000/api/users/all', { headers: { Authorization: 'Bearer ' + localStorage.getItem('token') } })
			.then(res => {
